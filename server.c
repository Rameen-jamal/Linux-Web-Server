...
// ========== AUTHENTICATION ==========
// Simple MD5-like hash function (not cryptographically secure, for demonstration purposes)
void simple_hash(const char *input, char *output) {
    unsigned char result[16];
    int i;
   
    // Simulate a hash function with simple operations
    for (i = 0; i < 16; i++) {
        result[i] = (input[i % strlen(input)] + i) % 256;
    }
...
int authenticate_user(const char *auth_header) {
    ...
    // For demonstration, we'll just use the passed string directly
    char decoded[128];
    strcpy(decoded, auth_header + 6); // Skip "Basic "
    ...
    char hashed_pwd[64];
    simple_hash(password, hashed_pwd);
...
void handle_http_request(int client_socket) {
    ...
    // Parse request
    char method[16], path[256], query[256] = "", auth_header[256] = "";
...
int check_deadlock() {
    ...
    for (int i = 0; i < THREAD_POOL_SIZE; i++) {
        if (thread_statuses[i].status == 2) { // If blocked
            int current = i;
